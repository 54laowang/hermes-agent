...
### Basic Task
```bash
~/.hermes/skills/turix-mac/scripts/run_turix.sh "Open Chrome and go to github.com"
```

### Resume Interrupted Task
```bash
~/.hermes/skills/turix-mac/scripts/run_turix.sh --resume my-task-001
```
...
### 1. 前置条件检查（Pre-flight Checklist）

**必须在执行任务前验证：**

| 检查项 | 验证命令 | 失败处理 |
|--------|---------|---------|
| 项目目录存在 | `test -d ~/.hermes/TuriX-CUA` | ❌ 终止，提示安装 |
| 配置文件存在 | `test -f examples/config.json` | ❌ 终止，提示创建 |
| Python venv 存在 | `test -f venv_turix/bin/python` | ⚠️ 警告，提示创建 |
| 屏幕录制权限 | `CGPreflightScreenCaptureAccess()` | ⚠️ 警告，提示授权 |
| PATH 包含 screencapture | `which screencapture` | ❌ 终止，设置 PATH |

**自动检查命令：**
```bash
~/.hermes/skills/turix-mac/scripts/run_turix.sh --dry-run "test"
```

### 2. 任务执行检查点（Task Execution Checkpoints）
...
# 检查最新日志时间戳
tail -1 ~/.hermes/TuriX-CUA/.turix_tmp/logging.log

# 检查步骤进度
ls ~/.hermes/TuriX-CUA/examples/.turix_tmp/brain_llm_interactions.log_brain_*.txt | wc -l
```
...
| 异常类型 | 症状 | 恢复方案 |
|---------|------|---------|
| **权限缺失** | `NoneType has no attribute 'save'` | 授权屏幕录制 → 重启终端 |
| **模型加载失败** | 进程卡住无日志 | 检查网络/模型文件 → 重试 |
| **内存不足** | 进程被杀（OOM） | 关闭其他应用 → 减少并发 |
| **任务卡死** | 步骤数不增长 > 5min | Cmd+Shift+2 停止 → 检查日志 |
| **目标未找到** | 重复点击无效 | 终止 → 重新描述任务 |
| **网络超时** | LLM 调用失败 | 检查网络 → 重试 |
...
```bash
# 任务中断后恢复
./run_turix.sh --resume my-task-001 "Continue from last step"
```

### 4. 边界条件与限制（Boundary Conditions）

**硬性边界：**

| 边界类型 | 限制 | 说明 |
|---------|------|------|
| **最大步数** | 100 步（默认） | 超出自动停止，可配置 |
| **单步操作数** | 5 个（默认） | 防止无限循环 |
| **模型加载时间** | 2-5 分钟（首次） | Brain/Actor/Memory 加载 |
| **任务超时** | 建议 30 分钟内 | 超时用 --resume 恢复 |
| **并发限制** | 单实例 | 不可同时运行多个 TuriX |
| **分辨率** | 支持任意 | 但高分屏可能影响识别准确度 |
...
| **复杂任务** | 拆分为多个子任务 | 提高成功率 |
| **中文路径** | 避免使用 | 可能导致编码问题 |
| **动态内容** | 增加等待时间 | 避免误判加载状态 |
| **多窗口** | 明确指定窗口名 | 防止操作错误窗口 |

**不适用场景（边界）：**
- ❌ 需要精确坐标的操作（TuriX 基于视觉理解）
- ❌ 实时性要求高的任务（模型推理有延迟）
- ❌ 需要大量文本输入（建议分段或用剪贴板）
- ❌ 系统级操作（需要 root 权限的操作）
- ❌ 后台任务（TuriX 需要前台可见窗口）

### 5. 任务后验证（Post-task Validation）
...
# 2. 检查日志末尾状态
tail -20 ~/.hermes/TuriX-CUA/.turix_tmp/logging.log | grep -E "completed|failed"

# 3. 检查生成的文件（如果有）
ls -lth ~/.hermes/TuriX-CUA/examples/.turix_tmp/*.txt | head -5
...
**成功标志：**
- ✅ 日志中出现 `✅ Task completed successfully`
- ✅ 无 `ERROR` 或 `CRITICAL` 级别日志
- ✅ 进程正常退出
- ✅ 预期结果已达成

**失败标志：**
- ❌ 日志中出现 `❌ Task failed` 或 `Max steps reached`
- ❌ 进程异常退出
- ❌ 步骤数达到上限仍未完成
...
```bash
# 方式1：使用 shell timeout（硬超时）
timeout 1800 ~/.hermes/skills/turix-mac/scripts/run_turix.sh "task"
# 1800秒 = 30分钟
...
if ps -p $TASK_PID > /dev/null; then
    echo "Task still running after 30min, consider stopping"
    # 可选择：kill $TASK_PID
fi
```
...
# 2. 记录 agent_id（从配置中获取）
grep agent_id ~/.hermes/TuriX-CUA/examples/config.json

# 3. 恢复任务
./run_turix.sh --resume <agent_id> "Continue task"
```

### 7. 状态管理与追踪（State Management）

**任务状态追踪：**

| 状态 | 标识 | 日志特征 |
|------|------|---------|
| **初始化** | INIT | `Loading models...` |
| **运行中** | RUNNING | `📍 Step N` |
| **暂停** | PAUSED | 无新日志 > 1min |
| **完成** | COMPLETED | `✅ Task completed` |
| **失败** | FAILED | `❌ Task failed` |
| **超时** | TIMEOUT | 进程仍在，无新步骤 |

**状态检查脚本：**
```bash
#!/bin/bash
# check_turix_status.sh

PID=$(ps aux | grep "python.*main.py" | grep -v grep | awk '{print $2}')
LOG_FILE="$HOME/.hermes/TuriX-CUA/.turix_tmp/logging.log"
...
STEP_COUNT=$(ls ~/.hermes/TuriX-CUA/examples/.turix_tmp/brain_llm_interactions.log_brain_*.txt 2>/dev/null | wc -l)

if echo "$LAST_LOG" | grep -q "completed"; then
    echo "状态: 已完成"
elif echo "$LAST_LOG" | grep -q "failed"; then
    echo "状态: 失败"
else
    echo "状态: 运行中（步骤数: $STEP_COUNT）"
fi
```
...
| 约束 | 说明 | 违反后果 |
|------|------|---------|
| **禁止删除操作** | 不执行 rm -rf 等危险命令 | 可能导致数据丢失 |
| **禁止系统配置修改** | 不修改系统设置 | 可能影响系统稳定性 |
| **禁止敏感信息输入** | 不输入密码/密钥 | 安全风险 |
| **确认重要操作** | 提交表单前需确认 | 避免误操作 |
| **限制文件访问** | 仅访问用户目录 | 保护系统文件 |
...
cd ~/.hermes/TuriX-CUA
~/.hermes/TuriX-CUA/venv_turix/bin/python examples/main.py
```
