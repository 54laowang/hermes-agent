...
description: >-
  Python/React 全栈项目的项目规划和功能分解。在规划阶段使用，
  将功能请求、用户故事或产品需求分解为实现计划。指导识别受影响的文件和模块、
  定义验收标准、评估风险并估算整体复杂度。生成模块图、风险评估和验收标准。
  不涵盖架构决策（使用 system-architecture）、实现（使用 python-backend-expert 或
  react-frontend-expert）或原子任务分解（使用 task-decomposition）。
license: MIT
