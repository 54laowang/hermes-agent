...
**上下文发现机制**：
- ✅ 触发词库配置（`~/.hermes/context_triggers.yaml`，4,704 字节）
- ✅ Shell Hook 实现（`~/.hermes/hooks/context_discovery.py`，9,522 字节）
- ✅ 三层架构：触发词扫描 → 语义检索 → 关联发现
- ⚡ 响应延迟：<500ms
- 📊 预期效果：Token 节省 30%，准确率 +20%

**触发词库覆盖**：
- 股票类：股价、行情、财报、技术分析等
- 技术分析类：趋势、均线、MACD、KDJ等
- 项目类：项目、代码、部署、测试等
- 记忆架构类：记忆、归档、fact_store、MemPalace等

**时间感知注入**：
- ✅ 自动注入当前时间上下文（精确到秒）
- ✅ 判断时间段（清晨/上午/中午/下午/晚上/深夜）
- ✅ 周末识别
- ✅ 行为准则（深夜提醒休息、周末理解非工作话题）
